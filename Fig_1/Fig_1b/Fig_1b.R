...
##==== Prepare data ====##
kendallRaster <- raster('Composite_36_Kendall_Ex_Greater0.tif')
...
##==== Draw ====##
Breaks <- c(min(resultMatrix$Value,na.rm = TRUE),100,200,300,400,500,600,max(resultMatrix$Value,na.rm = TRUE))
...
resultMatrix$Value <- factor(resultMatrix$Value,levels = c(0,1,2,3,4,5,6,7),ordered = TRUE)
# Draw
Fig_1b <- ggplot(data = resultMatrix,mapping = aes(x = Alt,y = Lat))+
  geom_tile(mapping = aes(fill = Value))+
  scale_fill_manual(values = c('#FFFFFF','#f5f487','#f3e276','#f4c76c','#f9a857','#f5864e','#ff5a3c','#f53132'))+
...
pdf(file = 'Fig_1b.pdf',width = 1.5,height = 2.64)
print(Fig_1b)
dev.off()
##==== Draw ====##