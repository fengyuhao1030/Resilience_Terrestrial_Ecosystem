...
##==== Prepare data ====##
kendallRaster <- raster('PRE_Total_Interpret_Ex.tif')
...
# Draw
Fig_2d <- ggplot(data = resultMatrix,mapping = aes(x = Alt,y = Lat))+
  geom_tile(mapping = aes(fill = Value))+
  scale_fill_manual(values = c('#FFFFFF','#d5e2ff','#adc9ff','#83b3f6','#5e9dfc','#4286f8'))+
...
pdf(file = 'Fig_2d.pdf',width = 1.5,height = 2.64)
print(Fig_2d)
dev.off()
##==== Draw ====##