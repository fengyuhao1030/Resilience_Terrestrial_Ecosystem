...
##==== Part 1: Prepare data ====##
kendallRaster <- raster('Proportion_Mean_Interpretation.tif')
...
Fig_3e <- ggplot(data = drawMatrix,mapping = aes(x = Class,y = Value))+
  geom_hline(yintercept = 0.5,linetype = 'dashed',color = '#000000',size = 0.4)+
  ...
  scale_y_continuous(limits = c(-0.1,1.2),breaks = seq(0,1.25,0.25),
                     labels = c('0','0.25','0.50','0.75','1.00','1.25'),expand = c(0,0))+
  ylab(expression(bold('Kendall¡¯s '*tau)))+
  theme_custom()+
  theme(axis.title.x = element_blank())
pdf(file = 'Fig_3e.pdf',width = 4.89,height = 2.3)
print(Fig_3e)
dev.off()
##==== Part 4: Draw ====##